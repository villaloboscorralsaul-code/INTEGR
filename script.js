...
function updatePreview() {
  ...
  previewNodes.pageLabel.textContent = "Documento de 2 páginas";
...
function renderPreviewItems(items) {
  const tbody = document.getElementById("previewConceptBody");
  if (!tbody) return;
  ...
  // Visual spacer rows for standard aesthetic
  const minRows = 3;
  const currentCount = items ? items.length : 0;
